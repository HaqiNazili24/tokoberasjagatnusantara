    ...
    <div class="offcanvas-lg offcanvas-start sidebar-admin p-0 flex-shrink-0" tabindex="-1" id="adminSidebar" style="width: 260px; background-color: #1E3932; color: white;" aria-labelledby="adminSidebarLabel">
        ...
        <div class="sidebar-brand px-4 py-4 d-flex align-items-center justify-content-between" style="border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
            <div class="d-flex align-items-center gap-2">
                <div class="sidebar-brand-icon" style="color: #cba258; font-size: 24px;">
                    <i class="bi bi-shield-lock-fill"></i>
                </div>
                <div>
                    <div class="sidebar-brand-name fw-bold" id="adminSidebarLabel" style="color: #ffffff; font-size: 18px;">Owner Panel</div>
        ...
        <nav class="sidebar-nav px-3 pb-3 flex-grow-1 mt-4">
            <div class="sidebar-section-label" style="color: rgba(255,255,255,0.4); font-size: 11px; font-weight: 700; letter-spacing: 1px; margin-bottom: 10px; padding-left: 10px;">MENU UTAMA</div>

            <a href="{{ route('owner.dashboard') }}" class="sidebar-link d-flex align-items-center gap-3 px-3 py-2 rounded mb-2 text-decoration-none {{ $r==='owner.dashboard' ? 'active' : '' }}" style="color: white; transition: all 0.2s;" data-tooltip="Dashboard">
                <span class="sidebar-link-icon"><i class="bi bi-speedometer2"></i></span>
                <span class="sidebar-link-text">Dashboard</span>
            </a>

            <a href="{{ route('owner.products.index') }}" class="sidebar-link d-flex align-items-center gap-3 px-3 py-2 rounded mb-2 text-decoration-none {{ str_starts_with($r,'owner.products') ? 'active' : '' }}" style="color: white; transition: all 0.2s;" data-tooltip="Produk">
                <span class="sidebar-link-icon"><i class="bi bi-box-seam"></i></span>
                <span class="sidebar-link-text">Kelola Produk</span>
            </a>

            <a href="{{ route('owner.users.index') }}" class="sidebar-link d-flex align-items-center gap-3 px-3 py-2 rounded mb-2 text-decoration-none {{ str_starts_with($r,'owner.users') ? 'active' : '' }}" style="color: white; transition: all 0.2s;" data-tooltip="Pengguna">
                <span class="sidebar-link-icon"><i class="bi bi-people-fill"></i></span>
                <span class="sidebar-link-text">Kelola Akun</span>
            </a>

            <a href="{{ route('owner.audit-logs') }}" class="sidebar-link d-flex align-items-center gap-3 px-3 py-2 rounded mb-2 text-decoration-none {{ $r==='owner.audit-logs' ? 'active' : '' }}" style="color: white; transition: all 0.2s;" data-tooltip="Audit Logs">
            ...
            <div class="sidebar-section-label" style="color: rgba(255,255,255,0.4); font-size: 11px; font-weight: 700; letter-spacing: 1px; margin-bottom: 10px; padding-left: 10px;">LAINNYA</div>

            <a href="{{ route('home') }}" class="sidebar-link d-flex align-items-center gap-3 px-3 py-2 rounded mb-2 text-decoration-none text-white" target="_blank" data-tooltip="Lihat Toko">
        ...
        <div class="sidebar-footer px-4 py-3" style="background-color: rgba(0, 0, 0, 0.2);">
            <div class="d-flex align-items-center gap-2">
                <div class="sidebar-avatar d-flex align-items-center justify-content-center fw-bold text-dark" style="width: 36px; height: 36px; border-radius: 50%; background-color: #cba258;">{{ strtoupper(substr(auth()->user()->full_name, 0, 1)) }}</div>
                <div>
                    <div class="sidebar-user-name fw-semibold" style="font-size: 14px;">{{ auth()->user()->full_name }}</div>
                    <div class="sidebar-user-role" style="font-size: 11px; color: #cba258;">Owner</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex-grow-1 w-100 admin-main" style="min-width: 0; background: #F2F0EB;">
        ...
        <header class="admin-topbar px-4 py-0 d-flex justify-content-between align-items-center sticky-top" style="height: 70px; background-color: white; border-bottom: 1px solid #edebe9; z-index: 1020;">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="mb-0 fw-bold" style="color: #006241;">@yield('page-title', 'Owner Dashboard')</h5>
                </div>
            </div>
            <div class="d-flex align-items-center gap-3">
...
<style>
    /* Styling rules mapped from DESIGN.md */
